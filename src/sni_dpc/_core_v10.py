...
def compute_composite_density(data, NN, RNN, M, S, G):
    ...
    views = np.vstack(
        [minmax_normalize(view) for view in (rho_nn, rho_rnn, rho_snn, rho_g)]
    )
    ...
    for i in range(n):
        ...
        for j in M[i]:
            ...
            if sij <= EPS:
                ...
                dij = euclidean_distance(data, i, j)
                scale = math.sqrt(local_scale[i] * local_scale[j]) + EPS
                d_scaled = dij / scale
...
def prototype_force_to_point(
    ...
    if not prototype:
        return 0.0
    dists = np.asarray(
        [euclidean_distance(data, point_i, int(j)) for j in prototype],
        dtype=float,
    )
        ...
        if gij is None:
            if reliability is not None and local_scale is not None and sigma is not None:
                ...
                if sij <= EPS:
                    ...
                    scale = (
                        math.sqrt(local_scale[point_i] * local_scale[j]) + EPS
                    )
                    d_scaled = dij / scale
                    alpha_ij = adaptive_alpha(
                        point_i,
                ...
                similarity = structure_similarity_from_gamma(
                    gamma, point_i, j
                )
                gij = similarity / (dij * dij + EPS)
        proto_wd = math.exp(-(dij * dij) / (proto_scale * proto_scale + EPS))
        mass = rho[j] * (1.0 + nb[j])
        if reliability is not None:
    ...
    while True:
        ...
        for i in candidates:
            ...
            similarity = len(gamma[i] & gamma_proto) / (
                len(gamma[i] | gamma_proto) + EPS
            )
            ratio = min(1.0, rho[i] / (rho[center] + EPS))
...
def layer3_gravity_boundary_assignment(
    ...
    for i in candidates:
        ...
        if force <= EPS:
            distance = min(
                euclidean_distance(data, i, int(p)) for p in prototype
            )
            force = 1.0 / (distance + EPS)
        current.append(force)
    positive = np.asarray([value for value in current if value > EPS])
    if not len(positive):
        return set()
    ...
    for i in candidates:
        ...
        for item in all_prototypes:
            ...
            if force <= EPS:
                distance = min(
                    euclidean_distance(data, i, int(p)) for p in item
                )
                force = 1.0 / (distance + EPS)
            forces.append(force)
